...
변수에도 타입할당이 가능한데 undefiend 또는 null 값만 할당할 수 있다.
...
누락된 값 이전 멤버값이 문자열이면 에러발생

enum은 양방향으로 접근이 가능하다.
...
```
역방향으로 접근할 경우 안전하지 않은 결과를 낳을 수 있다고 한다.
위의 역방향 접근을 막기 위한 const enum으로 열거형을 선언하는 방법이 있다.
...
cosnt array0:C = {value1: "문자임", value2: 123}  // 속성이 다있어도되고
cosnt array1:C = {value1: "문자임"} // 하나만 있어도 된다
cosnt array2:C = {value2: 123}

const func1 = (item:C): void => (
  item.value1  // 안된다
  item.value2 // 이것들은 속성이 겹치는게 없으니가 접근이안된다
//'C' 형식에 'value2' 속성이 없습니다.
//'A' 형식에 'value2' 속성이 없습니다.ts(2339)
)
